...
TIME_TO_SLEEP = 1.7 # Настройка паузы между запросами
PATH_TO_WEBDRIVER = '/Users/ilaantonov/PycharmProjects/parser/chromedriver' # Путь до Webdriver
SEARCH_FORMULA = 0.85*3 # Формула для поиска выгодных карт. (Изменять первый коэффициент)
...
def save_nameid(link, browser):
    browser.get(link)
    # time.sleep(TIME_TO_SLEEP)
    browser.find_element_by_class_name('market_listing_table_header').find_element_by_class_name(
        'market_listing_right_cell').click()
    ...
    for element in links_on_page:
        ...
        if element_name == 'Коллекционная карточка из ' + game_name:
            is_work_correct = False
            while is_work_correct == False:
                resp = requests.get(element['href'])
                time.sleep(TIME_TO_SLEEP)
                try:
                    item_nameid = re.search(r'Market_LoadOrderSpread\( \d+ \)', resp.text).group()[24:-2]
                except:
                    print('Parse item_nameid ERROR')
                    continue
                is_work_correct = True
            # print(item_nameid)
            game_items_nameid.append(item_nameid)
            print(game_items_nameid)
        if element_name == 'Набор карточек':
            ...
            while is_work_correct == False:
                ...
                try:
                    ...
                    continue
                is_work_correct = True
            game_items_nameid = [item_nameid] + game_items_nameid # мб трабл
            print(game_items_nameid)
    ...
    if len_links_on_page != game_items_count:
        for a in links_on_page:
            print(a.find('span', class_='market_listing_item_name').text, '\t',
                  a.find('span', class_='market_listing_game_name').text)
        raise Exception('Parsing wrong: game_items_count != len_links_on_page')
        return None

    return game_items_nameid
...
def should_to_bye(card_price, card_pak_price, count_set_on_market):
    if card_price == 0:
        return False
    elif (count_set_on_market > 10) & (card_price * SEARCH_FORMULA > card_pak_price):
        return True
    return False
...
def parse():
    ...
    with open('name_ids.txt', 'a') as fw:
        ...
        for index, link in enumerate(link_list[0:]):
            ...
            if should_to_bye(card_p, pack_p, count_set_on_market):
                game_items_nameid = save_nameid(link, browser)
                if game_items_nameid != None:
                    print(game_items_nameid)
                    game_items_string = ' '.join(game_items_nameid)
                    fw.write(game_items_string + '\n')
                else:
                    print('\n\n\nERROR\n\n\n')
    browser.quit()
