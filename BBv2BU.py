...
SHOULD_TO_BYE = False # Установить True для автоматической секупки паков
TIME_TO_SLEEP = 1.6 # Настройка паузы между запросами
PATH_TO_WEBDRIVER = '/Users/ilaantonov/PycharmProjects/parser/chromedriver' # Путь до Webdriver
SEARCH_FORMULA = 0.9*0.85*3 # Формула для поиска выгодных карт. (Изменять первый коэффициент)
...
def parse(link_list):
    ...
    with open('result.txt', 'w') as fw:
        ...
        for link in link_list:
            ...
            time.sleep(TIME_TO_SLEEP) # Задержка перед новым запросом для обхода бана (уточнить время!!!)
            print('--------------------NEW_LINK--------------------')
            browser.get(link)
