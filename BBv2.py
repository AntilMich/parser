...
TIME_TO_SLEEP = 2 # Настройка паузы между запросами
...
def parse_booster():
    ...
    browser = webdriver.Chrome(chrome_options=options, executable_path=PATH_TO_WEBDRIVER)
...
def should_to_bye(card_price, card_pak_price, count_cards_in_set):
    if card_price == 100:
        return False
    elif (count_cards_in_set > 10) & (card_price*SEARCH_FORMULA > card_pak_price):
        return True
    return False

def bye(link_to_bye, max_cost_of_set, browser = webdriver.Chrome(executable_path=PATH_TO_WEBDRIVER)): # Тут крашится
    for i in range(3):
        time.sleep(TIME_TO_SLEEP)
...
def login():
    ...
    browser.find_element_by_id('login_twofactorauth_buttonset_entercode').find_element_by_class_name('auth_button').click()
...
def parse(link_list):
    ...
    with open('result.txt', 'w') as fw:
        ...
        for link in link_list:
            ...
            time.sleep(TIME_TO_SLEEP) # Задержка перед новым запросом для обхода бана
            print('--------------------NEW_LINK--------------------')
            browser.get(link)
            ...
            for markets_element in markets_list:
                ...
                if 'Коллекционная карточка из ' in element_type:
                    # print(element_price)
            ...
            if should_to_bye(card_min_price, card_set_price, count_cards_in_set):
                ...
                bye(card_set_link, card_min_price*SEARCH_FORMULA, browser)
                fw.write(link+'\t\n')
    browser.quit()
...
# options = webdriver.ChromeOptions()
# options.add_argument('--headless')
# driver = webdriver.Opera(executable_path='/Users/ilaantonov/PycharmProjects/parser/operadriver', options=options)
